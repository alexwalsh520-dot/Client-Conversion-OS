...
type PanelClient = { key: string; name: string };
...
export default function OrganicKeywordsPanel() {
  ...
  const [clients, setClients] = useState<PanelClient[]>([{ key: "tyson", name: "Tyson" }]);
  const [loading, setLoading] = useState(true);
  const [client, setClient] = useState<string>("tyson");
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [kwRes, clRes] = await Promise.all([
        fetch("/api/organic-keywords", { cache: "no-store" }),
      ...
      if (Array.isArray(c?.clients) && c.clients.length > 0) {
        ...
        setClient((prev) => (c.clients.some((x: PanelClient) => x.key === prev) ? prev : c.clients[0].key));
      }
    } catch {
  ...
  return (
    <div className="glass-static" style={{ padding: 20, marginBottom: 20, borderLeft: "3px solid var(--accent)" }}>
      ...
      <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 16, lineHeight: 1.5 }}>
        Words people DM that are <b>not</b> from an ad (e.g. Tyson&apos;s <b>LOCKED</b>). The system auto-classifies these as organic so setters never have to attribute them by hand. An ad always wins while a paid ad with that word is live &mdash; a word only counts organic when no such ad is running.
      </div>
