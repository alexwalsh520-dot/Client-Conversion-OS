-- Convert spend to USD per Eastern-time day inside the aggregation, so every
-- money value the serving layer sees is already USD. Only non-USD creators
-- (Jake / AUD) are affected; USD creators pass through untouched. FX uses the
-- reference rate for the day the money moved, carried forward over weekends and
-- gaps (the latest rate on or before that day), matching src/lib/fx.
--
-- p_currency is a jsonb map {client_key: currency}, supplied by the serving
-- code which knows each creator's billing currency (creatorCurrency).
...
returns table (
  ...
  spend as (
    select client_key, kw,
      sum(
        case when ccy = 'USD' then spend_cents
