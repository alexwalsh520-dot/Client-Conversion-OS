-- Fold the ad creative preview into the leaves function so the serving layer
-- needs no separate (and expensive) per-window preview fetch. Latest non-null
-- preview per (client, keyword).
...
returns table (
  ...
  select k.client_key, k.kw, i.ad_id, i.ad_name, i.campaign_id, i.campaign_name, i.adset_id, i.adset_name,
    i.ad_effective_status, i.campaign_effective_status, p.preview_url,
