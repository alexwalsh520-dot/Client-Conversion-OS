...
For token-efficient browser work, keep structured captures in the browser-tool session and paste the serialized batch directly into **Paste a captured batch → Captured inbox batch** in CCOS, then click **Save captured batch**. Return only capture counts/checkpoints to the model while collecting; do not repeatedly print private transcripts into chat. Read rendered Everfit DOM through supported browser APIs; never extract cookies or call private Everfit APIs. This avoids saving 50 separate files or retyping each conversation. File import remains an alternative.
