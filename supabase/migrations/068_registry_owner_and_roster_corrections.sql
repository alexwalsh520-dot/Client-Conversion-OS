-- 068_registry_owner_and_roster_corrections.sql
-- TRUTH LAYER, Brick 1 addendum: owner-confirmed roster corrections (Alex, 2026-08-02).
--
-- 1. Matthew Conder is Alex's BUSINESS PARTNER and co-owner ("Matt"), not a
--    setter and not a creator. New entity kind 'owner'; canonical key renamed
--    setter_matthew_conder -> matthew_conder; status active.
-- 2. There is no closer named AARON. The name was a mis-transcription of ERIN,
--    the setter who also does outbound and closing. The closer_aaron row (empty
--    aliases, zero data references, created in 067) is removed; closer_roster
--    and setter_roster definitions are superseded by v2.
-- 3. Kelechi confirmed a current setter.
-- 4. Lucy confirmed a former client.
-- Seed functions are replaced with matching content so a reseed can never
-- resurrect the corrected rows. Idempotent: safe to run twice.

BEGIN;

ALTER TABLE public.registry_entities DROP CONSTRAINT IF EXISTS registry_entities_kind_check;
ALTER TABLE public.registry_entities ADD CONSTRAINT registry_entities_kind_check
  CHECK (kind = ANY (ARRAY['creator'::text, 'closer'::text, 'setter'::text, 'owner'::text]));

UPDATE public.registry_entities SET canonical_key = 'matthew_conder'
 WHERE canonical_key = 'setter_matthew_conder'
   AND NOT EXISTS (SELECT 1 FROM public.registry_entities WHERE canonical_key = 'matthew_conder');

DELETE FROM public.registry_entities WHERE canonical_key = 'closer_aaron';
...
AS $fn$
DECLARE
  v_signer     CONSTANT TEXT  := 'Alex Walsh (definitions signoff v3 + v4 roster corrections 2026-08-02, relayed via Fable)';
...
BEGIN
  ...
  ('lucy', 'Lucy Hubbard', 'creator', 'former', '2026-06-09',
   ...
   '{"ad_account": "USD", "revenue": "USD"}'::JSONB, NULL, NULL,
   'FORMER CLIENT, confirmed by Alex 2026-08-02. Evidence: client_key lucy in ads_meta_insights_daily (191 rows through 2026-06-09), ads_keyword_events (47), tracker offer "Lucy Hubbard" (4 rows), own ad account. The stale memory line "Lucy = NOT a client" is corrected. No dm_capture key exists for her.',
   v_signer, v_signed_at),
  ...
  ('setter_erin', 'Erin', 'setter', 'active', NULL,
   ...
   NULL, NULL, NULL,
   '92 tracker setter rows, 939 keyword events. DUAL ROLE CONFIRMED by Alex 2026-08-02: setter + outbound + closer; her 4 uppercase ERIN closer rows are real closes. Note: "AARON" on signoff v3 was a mis-transcription of Erin; no closer named Aaron exists.',
   v_signer, v_signed_at),
  ...
  ('setter_kelechi', 'Kelechi', 'setter', 'active', NULL,
   ...
   NULL, NULL, NULL,
   'CONFIRMED CURRENT by Alex 2026-08-02. Surfaced by the full-source sweep. 161 tracker setter rows, 397 keyword events. DUAL ROLE: also closer "KELECHI" on 19 rows.',
   v_signer, v_signed_at),

  -- Matthew Conder is Alex's business partner and co-owner (Alex, 2026-08-02).
  ('matthew_conder', 'Matthew Conder (Matt)', 'owner', 'active', NULL,
   '[{"system":"dm_capture","value":"matthew_conder"},
      {"system":"dm_setter","value":"Matthew Conder"}]'::JSONB,
   NULL, NULL, NULL,
   'OWNER: Alex''s business partner and co-owner ("Matt", co-CEO). Confirmed by Alex 2026-08-02, correcting both the signoff sheet (former creator) and the 067 seed (former setter). The DM evidence stands: 14 dm_conversation_messages rows keyed client=matthew_conder carry Tyson''s IG id (17841400987609293) or Antwan''s (17841401520122483) with setter_name="Matthew Conder" - he has worked DMs and the client column mis-keyed his name (data-quality item, rows untouched). No ad account, no ManyChat account, no tracker offer.',
   v_signer, v_signed_at)
...
BEGIN
  ...
  ('closer_roster', 1,
   ...
   v_signer, v_signed_at, 'superseded'),
  ...
  ('closer_roster', 2,
   'The closer roster is BROZ, WILL, AUSTIN, CHRIS, WOBBE. There is no closer named AARON: v1 recorded a mis-transcription of Erin, the setter who also does outbound and closing. Setters Amara, Kelechi, Debbie, Gideon and Erin appear in the closer column in uppercase and resolve to their setter entities; how commission math treats those rows is a Brick 2 question.',
   '{"closers": ["BROZ", "WILL", "AUSTIN", "CHRIS", "WOBBE"], "aaron": "never existed - mis-transcription of Erin (Alex 2026-08-02)", "erin_dual_role": true, "setter_names_in_closer_column": ["AMARA", "KELECHI", "DEBBIE", "GIDEON", "ERIN"]}'::JSONB,
   'Alex Walsh (v4 roster corrections 2026-08-02, relayed via Fable)', '2026-08-02', 'signed'),

  ('setter_roster', 2,
   'The setter roster is Amara, Erin, Debbie, Gideon, and Kelechi - all confirmed current by Alex 2026-08-02. Erin also does outbound and closing. The non-person tracker labels "Other" and "Phone Setter 1" map to no setter and must resolve to NULL. Roster and enumeration answers require a DISTINCT sweep of every source system, never a sample; rosters live in the registry.',
   '{"setters": ["Amara", "Erin", "Debbie", "Gideon", "Kelechi"], "pending_confirm": [], "erin_dual_role": true, "non_person_labels": ["Other", "Phone Setter 1"], "enumeration_rule": "full DISTINCT sweep of every source, never a sample"}'::JSONB,
   'Alex Walsh (v4 roster corrections 2026-08-02, relayed via Fable)', '2026-08-02', 'signed'),
