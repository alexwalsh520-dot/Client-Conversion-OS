...
-- Instagram account connections for live response-time tracking
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'instagram_connections' AND policyname = 'Allow service role manage instagram connections') THEN
    CREATE POLICY "Allow service role manage instagram connections" ON instagram_connections USING (true) WITH CHECK (true);
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'instagram_lead_links' AND policyname = 'Allow service role manage instagram lead links') THEN
    CREATE POLICY "Allow service role manage instagram lead links" ON instagram_lead_links USING (true) WITH CHECK (true);
  END IF;
END $$;
...
export async function GET() {
  return NextResponse.json({
    ...
    tables: [
      "manychat_tag_events",
      "manychat_contact_links",
      "dm_conversation_messages",
      "dm_conversation_stage_state",
      "instagram_connections",
      "instagram_lead_links",
      "report_history",
