...
function fakeDb(overrides: Record<string, unknown[]> = {}): { db: Db; calls: Calls } {
  ...
  const db = {
    ...
    from(table: string) {
      ...
      const chain = {
        ...
        // listLiveWindows resolves the builder directly (no maybeSingle).
...
const ask = (question_key: unknown, params: Record<string, unknown> = {}) =>
  askQuestion({ question_key, params }, { db: fakeDb().db, ...OPTS });
