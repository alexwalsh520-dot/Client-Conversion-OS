...
// Colored by the WORST status in the latest run; clicking opens /accuracy.
// Renders nothing until loaded and nothing on failure, so the tab is never
// blocked or broken by its own health indicator.
...
export default function AccuracyBadge() {
  ...
  return (
    <Link
      href="/accuracy"
      className="icon-btn"
      aria-label="Data accuracy checks"
      title={`Data accuracy: ${summary || "no checks yet"} (last run ${when}). Click for the daily checks.`}
    >
