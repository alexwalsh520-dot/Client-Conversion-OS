...
import { coachAlias, coachEmail } from "./coach-resolver";
...
/**
 * Post an auto-shipped plan to #nutritiontalk with the coach @mentioned.
 *
 * Looks up the coach's Slack user ID via their email (from the
 * coach-resolver roster). Falls back to admin DM if the Slack lookup
 * fails OR if NUTRITION_TALK_CHANNEL_ID env var isn't set — so the
 * plan still surfaces somewhere even in a half-configured prod env.
 */
export async function notifyNutritionTalkOfPlan(params: {
  runId: number;
  ...
  const channelId = process.env.NUTRITION_TALK_CHANNEL_ID;
  if (!channelId) {
    console.warn(
      "[notify-pipeline-result] NUTRITION_TALK_CHANNEL_ID not set; falling back to admin DM",
    );
    await notifyAdminTestRunDone({
      runId: params.runId,
      clientFullName: params.clientFullName,
      signedUrl: params.pdfSignedUrl,
      coachInternalName: params.coachInternalName,
    });
    return;
  }

  // Resolve coach Slack user ID
  const email = coachEmail(params.coachInternalName);
  let mention = params.coachInternalName ? `@${coachAlias(params.coachInternalName)}` : "team";
  if (email) {
    const slackId = await lookupUserIdByEmail(email);
    if (slackId) {
      mention = `<@${slackId}>`;
    }
  }

  const message = `${mention} ${params.clientFirstName}'s plan is ready. Please review, assign, and lmk when done.`;
  ...
  const blocks: Array<Record<string, unknown>> = [
      ...
      elements: [
        {
          type: "button",
          text: { type: "plain_text", text: "Download PDF", emoji: true },
          url: params.pdfSignedUrl,
          action_id: `download_auto_plan_${params.runId}`,
          style: "primary",
        },
      ],
    },
    {
      ...
      elements: [
        {
          type: "mrkdwn",
          text: `Auto-generated by CCOS pipeline. Run ID ${params.runId}. Already uploaded to the client's nutrition tab; this DM is just the heads-up + download link for Everfit.`,
        },
      ],
    },
  ];

  const fallback = `${params.clientFirstName}'s plan is ready: ${params.pdfSignedUrl}`;
  const result = await postBlocks(channelId, blocks, fallback);
  if (!result.ok) {
    console.warn(
      `[notify-pipeline-result] nutritiontalk post failed for run ${params.runId}:`,
      (result as { error?: string }).error ?? "unknown",
    );
    // Fall back to admin DM so the plan link doesn't disappear into the void
    await notifyAdminTestRunDone({
      runId: params.runId,
      clientFullName: params.clientFullName,
      signedUrl: params.pdfSignedUrl,
