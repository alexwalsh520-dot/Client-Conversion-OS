...
// setterKey -> Slack member ID. Update this when a setter joins/leaves the team.
// (Looked up from the Client Conversion Slack workspace.)
export const SETTER_SLACK_IDS: Record<string, string> = {
  amara: "U08JJ25GPBN",
  kelechi: "U0957LBTUV9",
  gideon: "U0946H0PD7V",
  debbie: "U09F2KQT52A",
...
// "<@ID>" if we have the owner's Slack ID, otherwise their plain name so the
// alert is still readable.
function ownerMention(owner?: string | null): string {
  const key = setterKey(owner);
  if (SETTER_SLACK_IDS[key]) return `<@${SETTER_SLACK_IDS[key]}>`;
  return SETTER_DISPLAY[key] || (owner?.trim() ? owner.trim() : "Unassigned");
}

// Every setter EXCEPT the owner — the ones invited to go steal the lead.
function otherSetterMentions(owner?: string | null): string {
  const ownerK = setterKey(owner);
  return Object.entries(SETTER_SLACK_IDS)
...
export function postTimeToEatAlert(leadName: string | null, owner: string | null) {
  const others = otherSetterMentions(owner);
  const text =
    `TIME TO EAT! ${others} GO HUNT!\n` +
    `Prospect: ${prospect(leadName)}\n` +
    `Current Owner: ${ownerMention(owner)}`;
  return postToSlack(CHANNEL, text);
}

/** A lead just landed in Dead Meat. */
export function postDeadMeatAlert(leadName: string | null, owner: string | null) {
  const text =
...
export function postAnswerLeadAlert(leadName: string | null, owner: string | null) {
  const text =
