...
import { loadHub, fmtDay, fmtDayYear, money, type HubClient, type Signal } from "@/lib/coaching-v2/hub";
  ...
  return (
    <>
      ...
      <div className="h2-cph">
        ...
        {c.convoId && <a className="h2-btn" href="https://app.everfit.io/" target="_blank" rel="noreferrer">Open in Everfit</a>}
      </div>

      <div className="h2-cp">
        <div>
          <div className="h2-blk h2-flags">
          ...
          {showRenewal && (
            <div className="h2-blk">
              <div className="h2-h">Retention</div>
              <div className="h2-ret">
                <span className="kk">Program ends</span>
                <span className={`v ${c.days! < 0 ? "h2-r" : c.days! <= 14 ? "h2-a" : ""}`}>{c.days! < 0 ? `Ended ${-c.days!} days ago, no decision recorded` : `In ${c.days} days, ${fmtDay(c.endDate)}`}</span>
        ...
        <div className="h2-facts">
          ...
          <div>
            ...
            {scores.length ? (
              <div className="h2-sc">{scores.map((k, i) => <i key={k.id} className={i === scores.length - 1 ? "last" : ""} style={{ height: Math.max(3, Math.round((k.score / 100) * 36)) }} title={String(k.score)} />)}</div>
            ) : null}
            <div className="h2-fact">
              ...
              <span className="kk">Count</span><span className="v">{c.checkins.length}</span>
            </div>
          </div>
          <div>
